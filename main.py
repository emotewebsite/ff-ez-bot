...
GUEST_UID = "4287206411"
GUEST_PASSWORD = "534E8411AF1334820FF02F01EFB90E6B6BC036688E94440018AFB0290AA5EF89"
region = 'IN'
...
def api_join():
    """Join team, send emote, leave"""
    tc = request.args.get('tc')
    uid1 = request.args.get('uid1')
    emote_id = request.args.get('emote_id')
    if not tc or not uid1 or not emote_id:
        return jsonify({'error': 'Missing parameters. Need tc, uid1, emote_id'}), 400

    if not main_asyncio_loop or not online_writer:
        return jsonify({'error': 'Bot not ready (no connection)'}), 503
    ...
    async def action():
        try:
            ...
            # Send emote
            emote_packet = await Emote_k(int(uid1), int(emote_id), bot_key, bot_iv, bot_region)
            if emote_packet:
            ...
            return {'success': True, 'message': f'Joined {tc}, sent emote {emote_id} to {uid1}, left'}
        except Exception as e:
            return {'error': str(e)}
    ...
    try:
        ...
        return jsonify({'error': f'Request timeout or error: {str(e)}'}), 500

@app.route('/emotes', methods=['GET'])
def api_emotes():
    """Return the list of emotes from emotes.json"""
    try:
        with open('emotes.json', 'r', encoding='utf-8') as f:
            emotes = json.load(f)
        return jsonify(emotes)
    except Exception as e:
        return jsonify({'error': f'Could not load emotes.json: {str(e)}'}), 500
...
async def MaiiiinE():
    ...
    global bot_key, bot_iv, bot_region, main_asyncio_loop, flask_thread
    ...
    # Set global variables for Flask API
    bot_key = key
    bot_iv = iv
    bot_region = region
